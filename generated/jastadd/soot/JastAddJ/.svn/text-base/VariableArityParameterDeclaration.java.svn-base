...
/*
A method is applicable if it is either applicable by subtyping (\u00df15.12.2.2),
applicable by method invocation conversion (\u00df15.12.2.3), or it is an applicable
variable arity method (\u00df15.12.2.4).

The process of determining applicability begins by determining the potentially
applicable methods (\u00df15.12.2.1). The remainder of the process is split into
three phases.

The first phase (\u00df15.12.2.2) performs overload resolution without permitting
...
The second phase (\u00df15.12.2.3) performs overload resolution while allowing
...
The third phase (\u00df15.12.2.4) allows overloading to be combined with variable
arity methods, boxing and unboxing.

Deciding whether a method is applicable will, in the case of generic methods
(\u00df8.4.4), require that actual type arguments be determined. Actual type
arguments may be passed explicitly or implicitly. If they are passed
implicitly, they must be inferred (\u00df15.12.2.7) from the types of the argument
expressions.
...
specified in section \u00df15.12.2.5. See the following subsections for details.
*/
